...
I was able to change the value but when I tried to click the button labeled `Respec` it wouldn't do anything. I again inspected using Dev Tools and found this had `disabled` and `cursor-not-allowed` modifiers. 
...
```
Now when clicking, this caused the `yes` popup to appear, but still didn't do anything. 
...
WIS: 55
```
It returned the starting values! If we can modify the `FLAG` value to 100, re-encode the value and set that as our cookie, we should get the flag! We can just do the oposite of the decoding we just did!
